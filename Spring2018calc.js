...
function readTrainingData(path) {
    return csv().fromFile(path).then((jsonObj) => {
        ...
        console.log(data)
            ...
            if (match.league === 'NALCS' && match.player != 'Team' && match.date > data && match.gameid !=1002620109 ) {
...
readTrainingData('./data2.csv').then((data)=>{
    //data[1][league]]
   ...
   // });
        var stream = fs.createWriteStream("stock.txt", {flags:'a'});
        var stockId =0;
